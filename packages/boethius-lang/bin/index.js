...
_commander2.default.version("0.0.1").option("-f, --file <file>", "File to parse").option("-i, --indir <indir>", "NOT YET IMPLEMENTED - Directory containing boethius files").option("-o, --outdir <outdir>", "Directory to output html file and assets").parse(process.argv);

var outdir = _commander2.default.outdir || './';

// If outdir doesn't exist then create it.
if (!_fs2.default.existsSync(outdir)) {
    _fs2.default.mkdirSync(outdir);
}

if (_commander2.default.file) {
    var bth = _fs2.default.readFileSync(_commander2.default.file, 'utf8');

    var _path$parse = _path2.default.parse(_commander2.default.file),
        name = _path$parse.name;

    var compiled = (0, _main2.default)(bth);

    _fs2.default.writeFileSync(_path2.default.join(outdir, name + '.json'), JSON.stringify(compiled, null, '\t'));

    console.log(_chalk2.default.green('Success!'));
} else if (_commander2.default.indir) {
    var dir = _fs2.default.readdirSync(_commander2.default.indir);
    console.log("Compiling Directory");
    dir.forEach(function (item) {
        var fullpath = _path2.default.join(_commander2.default.indir, item);
        if (_fs2.default.lstatSync(fullpath).isFile() && _path2.default.parse(item).ext === '.bth') {
            var _bth = _fs2.default.readFileSync(fullpath, 'utf8');

            var _path$parse2 = _path2.default.parse(item),
                _name = _path$parse2.name;

            var _compiled = (0, _main2.default)(_bth);

            _fs2.default.writeFileSync(_path2.default.join(outdir, _name + '.json'), JSON.stringify(_compiled, null, '\t'));
        }
    });
...
function compile(program, opts) {
    ...
    _lang.parser.yy.layout = new _Layout2.default();
    _lang.parser.yy.vars = {};
    _lang.parser.yy.BUILTINS = _builtins2.default;
    _lang.parser.yy.NumberNode = _NumberNode2.default;
    _lang.parser.yy.NoteNode = _NoteNode2.default;
    _lang.parser.yy.RestNode = _RestNode2.default;
    _lang.parser.yy.ScopeNode = _ScopeNode2.default;
...
rules: [/^(?:\s+)/,/^(?:;.*)/,/^(?:\|)/,/^(?:\()/,/^(?:\))/,/^(?:\[)/,/^(?:\])/,/^(?:\{)/,/^(?:\})/,/^(?:<)/,/^(?:>)/,/^(?:\/)/,/^(?:=)/,/^(?::)/,/^(?:r\b)/,/^(?:\.+)/,/^(?:"(.*?)")/,/^(?:true|false\b)/,/^(?:[0-9]+)/,/^(?:[a-gA-G][b|#]{0,2}(?![a-zA-Z])([0-9]+)?)/,/^(?:~[a-zA-Z][a-zA-Z0-9\-]*)/,/^(?:[a-zA-Z][a-zA-Z0-9]*)/,/^(?:$)/,/^(?:.)/],
 ...
 */
var octaveDirection = (0, _lodash2.default)(function (p1, octave, p2) {
    if (!p1) {
        return octave;
    }
    var n1 = _teoria2.default.note(p1 + octave);

    return (0, _lodash4.default)([octave, octave + 1, octave - 1], function (oct) {
        return Math.abs(_teoria2.default.interval(n1, _teoria2.default.note(p2 + oct)).semitones());
    });
}, function (p1, octave, p2) {
    return p1 + octave + p2;
});
...
function calculateTimes(phrase) {
	return _.map(_.reduce(phrase, function (acc, note) {
		var previous = _.last(acc);
		if (previous) {
			note.time = new _fraction2.default(previous.duration).add(previous.time);
		} else {
			note.time = 0;
		}
...
function accompany() {
	...
	return _.sortBy(_.flatten(phrases), function (n) {
		return n.time;
	});
}
...
function after(wait, notes) {
	var time = new _fraction2.default(wait);
...
function times(n, phrase) {
	return calculateTimes(_.flatten(_.times(n, function () {
...
function bpm(tempo) {
	return function (beat) {
		var time = new _fraction2.default(beat, tempo);
...
function duration(notes) {
	return _.sumBy(notes, function (n) {
		return n.duration;
	});
}
...
function rhythm() {
	...
	return phrase(durations, _.times(durations.length, function () {
		return null;
	}));
}
...
function inversion(chord, n) {
    var stable = _.take(_.keys(triad), n),
        lowered = _.difference(_.keys(chord), stable);

    var invertedChord = {};
    _.each(stable, function (k) {
        invertedChord[k] = chord[k];
    });

    _.each(lowered, function (k) {
...
function noteToMidi(note) {
    if ((0, _lodash.isString)(note)) {
        ...
        var _interval = NOTES[note.pitchClass];
        return octaveNoteToMidi(note.octave, _interval);
    }
}
...
var BUILTINS = {
    csym: function csym(yy, args) {
        var value = args[0].toString();
        var measure = args[1].value;
        var beat = args[2] ? args[2].value : 0;
        // const chordSymbol = {type: CHORDSYMBOL, props: {value, measure, beat}};
        var chordSymbol = new _ChordSymbol2.default({ value: value, measure: measure, beat: beat });

        return chordSymbol;
    },
    timesig: function timesig(yy, args) {
        var numerator = args[0].value;
        var denominator = args[1].value;
        ...
        var timeSignature = new _TimeSignature2.default(props);

        // yy.layout.timeSignatures.push(timeSignature);

        return timeSignature;
    },
    layout: function layout(yy, args) {
    ...
    page: function page(yy, args) {
        var page = new _PageNode2.default({
            systems: args[0].value
        });

        return page;
    },
    system: function system(yy, args) {
        var _args = _toArray(args),
            measures = _args[0],
            lineSpacings = _args.slice(1);
    ...
    line: function line(yy, args) {
        var props = args.reduce(function (acc, arg) {
            if (arg instanceof _Keyword2.default) {
                acc.voices.push(arg.toString());
            } else if (arg.type === _constants.CLEF) {
                acc.clefs.push(arg);
            } else if (arg.type === _constants.KEY) {
                acc.keys.push(arg);
            }
            return acc;
        }, { keys: [], clefs: [], voices: [] });

        var line = new _LineNode2.default(props);
    ...
    clef: function clef(yy, args) {
        ...
        var beat = args[2] ? args[2].value : 0;
        return { type: _constants.CLEF, value: value, measure: measure, beat: beat };
    },
    key: function key(yy, args) {
        ...
        var measure = args[2] ? args[2].value : 0;
        var beat = args[3] ? args[3].value : 0;
        return { type: _constants.KEY, root: root, mode: mode, measure: measure, beat: beat };
    }
};
